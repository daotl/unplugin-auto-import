...
var _chunkKYJK6LVWcjs = require('./chunk-KYJK6LVW.cjs');
    ...
    if (dts) {
      promises.push(
        generateDTS(dts).then((content) => {
          if (content !== lastDTS) {
            lastDTS = content;
            return _fs.promises.writeFile(dts, content, "utf-8");
          }
        })
      );
    }
      ...
      promises.push(
        generateESLint().then((content) => {
          if (content !== lastESLint) {
            lastESLint = content;
            return _fs.promises.writeFile(eslintrc.filepath, content, "utf-8");
          }
        })
      );
    }
...
function flattenImports(map, overriding = false) {
  ...
  _utils.toArray.call(void 0, map).forEach((definition) => {
    if (typeof definition === "string") {
      if (!_chunkKYJK6LVWcjs.presets[definition])
        throw new Error(`[auto-import] preset ${definition} not found`);
      const preset = _chunkKYJK6LVWcjs.presets[definition];
